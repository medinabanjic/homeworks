**Zadatak 1**

Napisati dvije klase:

1. StoryElement klasa (abstract)

2. Character klasa koja nasljeðuje StoryElement klasu

a. Klasa koja opisuje jedan element u radnji jedne prièe

b. Sadrži atribut imena

c. Sadrži dva konstruktora

i. Konstruktor koji inicijalizira ime na datu vrijednost

ii. Prazni konstruktor koji inicijalizira, preko prvog konstruktora, ime na „STElement“

a. Klasa koja opisuje jednog lika u radnji jedne prièe

b. Sadrži spol karaktera

c. Sadrži atribut koji opisuje da li je lik živ ili ne (default je true)

d. Sadrži atribut koji opisuje da li je lik sposoban ili ne

e. Sadrži metodu koja... ubija lika

f. Sadrži metodu koja pretvara nesposobnog lika u sposobnog

g. Konstruktor koji inicijalizira ime, spol karaktera i sposobnost karaktera



**Zadatak 2**

Napisati dvije klase:

1. Event klasa

2. Goal klasa koja nasljeðuje StoryElement klasu

a. Klasa koja opisuje dogaðaj u radnji jedne prièe

b. Klasa sadrži samo konstante i to:

i. Poraz

ii. Prijateljstvo

iii. Ljubav

iv. Osveta

v. Manipulacija

a. Klasa koja opisuje cilj jednog lika u radnji jedne prièe

b. Sadrži atribut težine bazirano na tri vrijednosti (lagano, teško i nemoguæe)

c. Sadrži atribut koji opisuje šta (Event) se treba desiti da se ispuni cilj

d. Sadrži atribut koji opisuje kome ili sa kim (Character) se treba desiti dogaðaj pod c.

e. Sadrži konstruktor koji inicijalizira tri pomenuta atributa, kao i ime cilja



**Zadatak 3**

Napisati klasu:

1. Protagonist klasa koja nasljeðuje Character klasu

a. Opisuje glavnog pozitivca u prièi

b. Sadrži atribut koji opisuje glavni cilj (Goal) lika

c. Sadrži jedan niz, kao atribut, likova (Character) koji pomažu glavnom liku (max. 3)

d. Sadrži jedan atribut koji oznaèava glavnog neprijatelja (Character)

i. Dati atribut može biti null

e. Sadrži konstruktor koji inicijalizira ime, spol, sposobnost i cilj lika

f. Sadrži metodu koja postavi glavnog neprijatelja

g. Sadrži metodu koja doda jednog lika koji æe pomoæi glavnom liku

i. Pomenuta metoda se može samo èetiri puta pozvati (peti throw-a exception) 



**Zadatak 4**

Napisati metodu:

1. Metoda attainGoal() u klasi Protagonist

a. Metoda koja pokrene ciklus borbe glavnog lika i pokušaja da dokuèi svoj cilj

b. Metoda ne može raditi ukoliko glavni lik nije živ

c. Metoda izraèuna kolika je šansa da glavni lik dokuèi svoj cilj

i. Ukoliko je glavni lik sposoban onda +40%

ii. Za svakog sposobnog pomoænog lika +20%

iii. Ukoliko je glavni neprijatelj sposoban onda -30%

iv. Ukoliko je cilj težak onda -20%

v. Ukoliko je cilj nemoguæ onda -100%

vi. Šansa uvijek mora biti izmeðu 0 i 100%

d. Bazirano na izraèunatoj šansi metoda odluèi da li je lik uspio ili ne

i. Uspjeh:

ii. Neuspjeh:

1. Ispisati na konzolu da je lik uspio i u èemu (Goal) je uspio

2. Napraviti jednog lika (nasumièno izabranog) sposobnim (ako nije bio)

1. Ispisati na konzolu da lik nije uspio i u èemu (Goal) nije uspio

2. Nasumièno izabrati jednoga lika koji je uèestvovao i ubiti ga

3. Napraviti jednog lika (nasumièno izabranog) sposobnim (ako nije bio)



**Zadatak 5**

Napraviti prièu u main metodi pomoæu navedenih klasa!